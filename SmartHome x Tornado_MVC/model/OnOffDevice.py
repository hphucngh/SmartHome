    ...
    def post(self):
        ...
        def writeTemperatureHumidity():
            ...
            while True:
                ...
                if humidity is not None and temperature is not None:
                    new_status = {"name": "temperature-humidity-bedroom", "temperature": temperature, "humidity": humidity, "time": datetime.datetime.utcnow(), "iddevice": "temperaturehumidityBedRoom"}
        ...
        if key == "fan-living-room":
            if status == "on":
                ...
                new_status = {"name": key, "status": status, "data": "on device", "starttime": starttime, "endtime": None, "iddevice": "fanLivingRoom"}
                Env.database["statusdb"].insert(new_status)  # Insert du lieu new_status vao Collection statusdb
            ...
            if status == "on":
                ...
                self.write("true")  # Tra ve clien "true"
                new_status = {"name": key, "status": status, "data": "on device", "starttime": starttime, "endtime": None, "iddevice": "fanBedRoom"}
                Env.database["statusdb"].insert(new_status)  # Insert du lieu new_status vao Collection statusdb
