	...
	def post(self):
		...
		if key == "auto-humidity-land":
			...
			if status == "off":
				...
				keyWater = "water-tree"
				finds = json.loads(dumps(Env.database["statusdb"].find({"name":keyWater},{"status":1,"_id":0}).sort([("_id", -1)]).limit(1)))[0]
                                f = finds["status"]
                                Env.database["statusdb"].update({"name":keyWater,"status":f},{"$set":{"endtime":endtime,"status":status}})